...
| `matheo` | `neutral`, `read`, `hollow` | pronto (`size: full`) |
...
| `dara` | `dara_neutral.png` | 320×340 bust | Copa, Capítulo 2 |
| `dara` | `dara_guarded.png` | 320×340 bust | Quando a conversa chega perto do lado norte |
| `ren` | `ren_neutral.png` | 320×340 bust | Biblioteca, Capítulo 2 |
...
Todos existem como `.wav`, sintetizados em `tools/make_sfx.py`. Para
substituir por gravação, basta colocar o arquivo com o mesmo nome — o engine
testa `.wav`, depois `.ogg`, depois `.mp3`, e usa o primeiro que carregar.

| ID lógico | Situação | Onde entra |
|---|---|---|
| `sfx_package` | pronto | Pacote pousando na mesa. Discreto, com peso. |
| `sfx_page_turn` | pronto | Página virada |
| `sfx_candle` | pronto | Vela apagando |
| `sfx_door` | pronto | Portão de ferro, **sem rangido** |
| `sfx_footsteps` | pronto | Passos lentos e regulares em madeira |
| `sfx_wind` | pronto | Vento na estrada |
...
## Regerar os efeitos sonoros

```bash
python3 tools/make_sfx.py
```

Só biblioteca padrão do Python. Grava WAV mono 44.1 kHz 16 bits em
`assets/audio/sfx/`. Editar os parâmetros de síntese no próprio script muda o
resultado; regerar sobrescreve.
